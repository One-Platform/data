 ...
 */
class OneTransactionalRepositoryProxyPostProcessor implements RepositoryProxyPostProcessor {
	 ...
	 */
	public OneTransactionalRepositoryProxyPostProcessor(ListableBeanFactory beanFactory, String transactionManagerName, SqlQueries sqlQueries) {
		...
		this.transactionInterceptor = new TransactionInterceptor(null, new OneCustomAnnotationTransactionAttributeSource(sqlQueries));
	 ...
	 */
	static class OneCustomAnnotationTransactionAttributeSource extends OneAbstractFallbackTransactionAttributeSource implements
			Serializable {
		...
		private static final boolean ejb3Present = ClassUtils.isPresent("javax.ejb.TransactionAttribute",
                OneCustomAnnotationTransactionAttributeSource.class.getClassLoader());
		 ...
		 */
		public OneCustomAnnotationTransactionAttributeSource(boolean publicMethodsOnly) {
		 ...
		 */
		public OneCustomAnnotationTransactionAttributeSource(TransactionAnnotationParser annotationParser) {
		 ...
		 */
		public OneCustomAnnotationTransactionAttributeSource(Set<TransactionAnnotationParser> annotationParsers) {
			this.publicMethodsOnly = true;
		 ...
		 * <p>
		 * This implementation delegates to configured {@link org.springframework.transaction.annotation.TransactionAnnotationParser TransactionAnnotationParsers} for
	...
	/**
	 * Abstract implementation of {@link org.springframework.transaction.interceptor.TransactionAttributeSource} that caches attributes for methods and implements a
	 ...
	 * @since 1.1
	 */
	abstract static class OneAbstractFallbackTransactionAttributeSource implements TransactionAttributeSource {
            ...
            if (cached != null) {
                ...
                if((isSQLQuery = isSQLQuery(method)) || (isSQLAnnotation = isSQLAnnotation(method))) {
                    ...
                    try {
                        ...
                        logger.warn("no [update] method adapter. exception messager is : " + e.getLocalizedMessage());
        ...
        private boolean isSQLAnnotation(Method method) {
            return method.getAnnotation(SQL.class) != null;
        }

        private SQLType getSqlType(String sql) {
            SQLType sqlType = SQLType.AUTO_DETECT;
            for (int i = 0, len = StatementMetaData.SELECT_PATTERNS.length; i < len; i++) {
                // 用正则表达式匹配  SELECT 语句
