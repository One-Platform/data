 ...
 */
class OneRepositoryInterfaceAwareBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter implements
		BeanFactoryAware {

	private static final Log LOG = LogFactory.getLog(OneRepositoryInterfaceAwareBeanPostProcessor.class);
	private static final Class<?> REPOSITORY_TYPE = OneRepositoryFactoryBeanSupport.class;
	...
	/**
	 * Returns the class which is configured in the given {@link org.springframework.beans.PropertyValue}. In case it is not a
	 * {@link org.springframework.beans.factory.config.TypedStringValue} or the value contained cannot be interpreted as {@link Class} it will return null.
	 * 
	 * @param propertyValue
	 * @param beanName
	 * @return
	...
	private Class<?> getClassForPropertyValue(PropertyValue propertyValue, String beanName) {
		...
		try {
			return ClassUtils.resolveClassName(className, OneRepositoryInterfaceAwareBeanPostProcessor.class.getClassLoader());
